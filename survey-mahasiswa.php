...
if(isset($_POST['mahasiswa'])) {
  ...
  $inserdata = $biodata->insertBiodata("mahasiswa", $data);
...
if (isset($_GET['submit'])) {
  ...
  if($responden->insertData("mahasiswa", $dataTemporary)){
    echo "<script>
alert('Data kamu berhasil kami simpan!');
              window.location = 'biodata.php?kategori=mahasiswa';
</script>";
  }
    ...
    <section class="content">
      <div class="card">
        <div class="card-header bg-primary">
          <h3 class="card-title font-weight-bold">Form Survey Mahasiswa</h3>
        </div>
        <?php
        if (isset($_GET['submit'])) {
          ...
          foreach ($soal as $row):
            ...
            <div>
              <p><?= $no . ". " . $row["soal_nama"] ?></p>
                  ...
                  <input type="text" class="d-none" name="responden_mahasiswa_id_<?= $no ?>" value="<?= $row['id_responden'] ?>">
              <?php
