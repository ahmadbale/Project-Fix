...
if(isset($_POST['industri'])) {
  ...
  $inserdata = $biodata->insertBiodata("industri", $data);
...
if (isset($_GET['submit'])) {
  ...
  if($responden->insertData("industri", $dataTemporary)){
    echo "<script>
alert('Data kamu berhasil kami simpan!');
              window.location = 'biodata.php?kategori=industri';
</script>";
  }
...
<head>
  ...
  <title>Survey Polinema | Industri</title>
    ...
    <section class="content">
      <div class="card">
        <div class="card-header bg-primary">
          <h3 class="card-title font-weight-bold">Form Survey Industri</h3>
        </div>
        <?php
        if (isset($_GET['submit'])) {
          ...
          foreach ($soal as $row):
            ...
            <div>
              <p><?= $no . ". " . $row["soal_nama"] ?></p>
                  ...
                  <input type="text" class="d-none" name="responden_industri_id_<?= $no ?>" value="<?= $row['id_responden'] ?>">
              <?php
