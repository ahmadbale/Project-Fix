...
if(isset($_POST['tendik'])) {
  ...
  $inserdata = $biodata->insertBiodata("tendik", $data);
...
if (isset($_GET['submit'])) {
  ...
  array_push($dataTemporary, $dataInput);
  if($responden->insertData("tendik", $dataTemporary)){
    echo "<script>
alert('Data kamu berhasil kami simpan!');
              window.location = 'biodata.php?kategori=tendik';
</script>";
  }
    ...
    <section class="content">
      <div class="card">
        <div class="card-header bg-primary">
          <h3 class="card-title font-weight-bold">Form Survey Tenaga Pendidik</h3>
        </div>
        <?php
        if (isset($_GET['submit'])) {
          ...
          foreach ($soal as $row):
            ...
            <div>
              <p><?= $no . ". " . $row["soal_nama"] ?></p>
                  ...
                  <input type="text" class="d-none" name="responden_tendik_id_<?= $no ?>" value="<?= $row['id_responden'] ?>">
              <?php
